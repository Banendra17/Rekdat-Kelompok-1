...
dag = DAG(
    ...
    description='twitter scrapping DAG 2',
...
def get_search(**kwargs):
    ...
    q = "#SpotifyWrapped"
    api = get_auth()
    cursor = tweepy.Cursor(api.search_tweets, q=q).items(1000)
    c = 0
    for x in cursor:
        c = c + 1
        print(x)
...
csv_path = Path("/opt/airflow/data/tweets_search2.csv")
...
def addtodb():
    ...
    with open('/opt/airflow/data/tweets_search2.csv', 'r') as f:
        reader = csv.reader(f)
        next(reader)
        for row in reader:
            cursor.execute(
                "INSERT INTO tweets_search2 VALUES (%s, %s, %s, %s, %s)",
                row
            )
    conn.commit()
